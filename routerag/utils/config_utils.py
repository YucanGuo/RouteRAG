...
class BaseConfig:
    ...
    reader_name: str = field(
        default="gpt-4o-mini",
        metadata={"help": "Class name indicating which reader LLM model to use."}
    )
    reader_base_url: str = field(
        default=None,
        metadata={"help": "Base URL for the reader LLM model, if none, means using OPENAI service."}
    )
    ...
    stop_words: Optional[List[str]] = field(
        default=None,
        metadata={"help": "List of stop words to control generation. If None, no stop words are used."}
    )
    keep_stop_words: bool = field(
        default=True,
        metadata={"help": "Whether to keep stop words in the final output. If True, stop words will be preserved."}
    )
