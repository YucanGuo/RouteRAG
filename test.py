...
from routerag.utils.misc_utils import string_to_bool
from routerag.utils.config_utils import BaseConfig
...
def main(): 
    parser = argparse.ArgumentParser(description="RouteRAG retrieval and QA")
    parser.add_argument('--dataset', type=str, default='musique', help='Dataset name')
    parser.add_argument('--llm_base_url', type=str, default='https://api.openai.com/v1', help='LLM base URL')
    parser.add_argument('--llm_name', type=str, default='gpt-4o-mini', help='LLM name')
    parser.add_argument('--reader_base_url', type=str, default='https://api.openai.com/v1', help='reader LLM base URL')
    parser.add_argument('--reader_name', type=str, default='gpt-4o-mini', help='reader LLM name')
    ...
    parser.add_argument('--deepsearch', action="store_true", default=False, help='enable deepsearch')
    ...
    samples = json.load(open(f"dataset/{dataset_name}.json", "r"))
    ...
    config = BaseConfig(
        ...
        rerank_dspy_file_path="routerag/prompts/dspy_prompts/filter_llama3.3-70B-Instruct.json",
        retrieval_top_k=200,
        linking_top_k=5,
        max_qa_steps=4,
        qa_top_k=3,
    ...
    reader_name = "_".join(args.reader_name.split('/')[-3:])
    log_name = f"{save_dir}/" + llm_name.split('/')[-1].replace('-', '_') + '_' + reader_name.replace('-', '_') + '_' + str(args.deepsearch) + "_hybrid.log"
    ...
    routerag = RouteRAG(global_config=config)

    routerag.index(docs)

    # Retrieval and QA
    enable_deepsearch = args.deepsearch
    # all_queries, gold_docs, gold_answers = all_queries[:50], gold_docs[:50], gold_answers[:50]
    routerag.rag_qa(queries=all_queries, gold_docs=gold_docs, gold_answers=gold_answers, deepsearch=enable_deepsearch)
