...
import re
from routerag.llm_agent.generation import LLMGenerationManager, GenerationConfig
...
def compute_data_metrics(batch, use_critic=True):
    ...
    if 'search_times' in batch.meta_info:
        metrics['env/search_time/mean'] = float(np.array(batch.meta_info['search_times'], dtype=np.int16).mean())
        metrics['env/search_time/max'] = float(np.array(batch.meta_info['search_times'], dtype=np.int16).max())
        metrics['env/search_time/min'] = float(np.array(batch.meta_info['search_times'], dtype=np.int16).min())
    # metrics for EM scores
    if 'em_scores' in batch.meta_info:
        em_scores = np.array(batch.meta_info['em_scores'])
        metrics['em_score/mean'] = float(em_scores.mean())
        metrics['em_score/max'] = float(em_scores.max())
        metrics['em_score/min'] = float(em_scores.min())
    return metrics
...
class RayPPOTrainer(object):
    ...
    def _create_dataloader(self):
        ...
        with open_dict(self.config):
            ...
            if self.config.trainer.resume_from_steps:
                self.config.actor_rollout_ref.actor.optim.resume_from_steps = self.config.trainer.resume_from_steps
                self.config.critic.optim.resume_from_steps = self.config.trainer.resume_from_steps
    ...
    def _save_checkpoint(self):
        cur_steps = self.global_steps
        actor_local_path = os.path.join(self.config.trainer.default_local_dir, 'actor',
                                        f'global_step_{cur_steps}')
    ...
    def fit(self):
        ...
        logger = self.logger
        resume_from_steps = self.config.trainer.get('resume_from_steps', 0)
        if resume_from_steps > 0:
            self.global_steps = resume_from_steps
            print(f'Resuming training from step {resume_from_steps}')
        else:
            self.global_steps = 0
        ...
        if resume_from_steps > 0:
            start_epoch = resume_from_steps // steps_per_epoch
            start_batch_idx = resume_from_steps % steps_per_epoch
            print(f'Resuming from epoch {start_epoch}, batch {start_batch_idx} (total steps: {resume_from_steps})')
        else:
            start_epoch = 0
            start_batch_idx = 0
        ...
        for epoch in range(start_epoch, self.config.trainer.total_epochs):
            for batch_idx, batch_dict in enumerate(self.train_dataloader):
                ...
                if self.global_steps >= self.total_training_steps:
                    print(f'Training completed at step {self.global_steps}')
                    self._save_checkpoint()
