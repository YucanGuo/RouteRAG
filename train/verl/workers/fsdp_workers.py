...
class ActorRolloutRefWorker(Worker):
    ...
    def _build_model_optimizer(self,
        ...
        if self._is_actor:
            ...
            resume_from_steps = optim_config.get('resume_from_steps', 0)

            print(f'Total steps: {total_steps}, num_warmup_steps: {num_warmup_steps}, resume_from_steps: {resume_from_steps}')

            actor_lr_scheduler = get_constant_schedule_with_warmup(optimizer=actor_optimizer,
                                                                   num_warmup_steps=num_warmup_steps,
                                                                   last_epoch=resume_from_steps-1 if resume_from_steps > 0 else -1)
        else:
